"""Shared static server-menu extraction for WordPress episode sources."""
import asyncio
import base64
import re
...
def metadata(label: str) -> tuple[str, list[str]]:
    ...
    server = next((name for pattern, name in [
        ...
        (r'vid[\s_-]*hide', 'VidHide'), (r'with\s+ads', 'With Ads')
    ] if re.search(pattern, lower)), label)
    return server, languages


def choices(html: str, base: str, normalize_url) -> tuple[str, list[dict], bool]:
    soup = BeautifulSoup(html, 'html.parser')
    ...
    for node in nodes:
        ...
        if raw.isdigit():
            ...
            try:
                if normalize_url(candidate) != base:
                    continue
            except ValueError:
                continue
    ...
    return heading.get_text(' ', strip=True) if heading else 'Episode', rows[:64], truncated


async def extract(url: str, fetch, *, normalize_url, source: str) -> dict:
    ...
    if urlsplit(url).query:
        ...
        for candidate in candidates:
            try:
                resolved = normalize_url(candidate)
    ...
    title, rows, truncated = choices(html, url, normalize_url)
    ...
    return dict(source=source, url=url, title=title, servers=rows, warnings=warnings,
